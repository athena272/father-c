...
// Função que verifica se duas árvores binárias são iguais através de busca em profundidade (DFS).
// Recebe como parâmetro duas árvores binárias.
// Retorna true se as árvores são iguais e false caso contrário.
bool iguais(ARVORE_BINARIA *arv1, ARVORE_BINARIA *arv2)
{
    ...
    // Se uma das árvores é nula e a outra não é, elas são diferentes
    if (arv1 == NULL || arv2 == NULL)
        return false;
    ...
    // Chama a função recursivamente para os filhos da esquerda e da direita
    bool iguaisEsquerda = iguaisDFS(arv1->esq, arv2->esq);
    bool iguaisDireita = iguaisDFS(arv1->dir, arv2->dir);

    // Retorna true se ambos os filhos são iguais
