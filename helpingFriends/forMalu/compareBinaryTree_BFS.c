...
bool iguais(ARVORE_BINARIA *arv1, ARVORE_BINARIA *arv2)
{
    ...
    inserirFila(&fila1, arv1);
    inserirFila(&fila2, arv2);

    while (!vaziaFila(&fila1) && !vaziaFila(&fila2))
    {
        ARVORE_BINARIA *no1 = removerFila(&fila1);
        ARVORE_BINARIA *no2 = removerFila(&fila2);

        if (no1->item.chave != no2->item.chave)
        {
            liberarFila(&fila1);
            liberarFila(&fila2);
            return false;
        ...
        if (no1->esq != NULL && no2->esq != NULL)
        {
            inserirFila(&fila1, no1->esq);
            inserirFila(&fila2, no2->esq);
        }
        else if (no1->esq != NULL || no2->esq != NULL)
        {
            liberarFila(&fila1);
            liberarFila(&fila2);
            return false;
        ...
        if (no1->dir != NULL && no2->dir != NULL)
        {
            inserirFila(&fila1, no1->dir);
            inserirFila(&fila2, no2->dir);
        }
        else if (no1->dir != NULL || no2->dir != NULL)
        {
            liberarFila(&fila1);
            liberarFila(&fila2);
            return false;
