...
bool eBinariaBusca(ARVORE_BINARIA *arvore)
{
    ...
    if (!vazia(arvore->esq))
    {
        // Verifica se a subárvore esquerda não é uma árvore de busca binária
        // ou se a chave da subárvore esquerda é maior ou igual à chave da raiz
        if (!eBinariaBusca(arvore->esq) || arvore->esq->item.chave >= arvore->item.chave)
            return false;
    }
    ...
    if (!vazia(arvore->dir))
    {
        // Verifica se a subárvore direita não é uma árvore de busca binária
        // ou se a chave da subárvore direita é menor ou igual à chave da raiz
        if (!eBinariaBusca(arvore->dir) || arvore->dir->item.chave <= arvore->item.chave)
            return false;
    }

    // Se chegou até aqui, a árvore é uma árvore de busca binária
    return true;
}
