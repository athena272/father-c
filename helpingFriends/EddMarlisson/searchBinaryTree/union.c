...
bool uniao(ARVORE_BINARIA *arv1, ARVORE_BINARIA *arv2, ARVORE_BINARIA **arvU)
{
    ...
    // Verificar se a união pode ser feita
    if (existeIntersecao(arv1, arv2))
        return false;

    // Inicializar a árvore de união
    inicializar(arvU);

    // Percorrer a primeira árvore e inserir os nós na árvore de união
    copiarArvore(arv1, arvU);

    // Percorrer a segunda árvore e inserir os nós que não estão na árvore de união
    copiarNaoRepetidos(arv2, arvU);
