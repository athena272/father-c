...
bool iguais(ARVORE_BINARIA *arv1, ARVORE_BINARIA *arv2)
{
    ...
    // se ambas as arvores estao vazias, elas sao iguais
    if (vazia(arv1) && vazia(arv2))
    {
        return true;
    }
    // se apenas uma das arvores esta vazia, elas nao sao iguais
    if (vazia(arv1) || vazia(arv2))
    {
        return false;
    }
    // se as chaves dos nos sao diferentes, as arvores nao sao iguais
