...
{
    // Retirei aqui o if que return false, como a funcao é void, isso geraria um erro e não faz sentido
    NO *pAnterior = l->cabeca;
    NO *pAtual = NULL;

    // Ao inves de pAtual = pAnterior->proxima
    // Criei um ponteiro extra
    NO *proximo;

    while (pAnterior != NULL) { 
        // usei o ponteiro "proximo" para armazenar o ponteiro para o próximo nó da lista, que eh o proximo do pAnterior
        proximo = pAnterior->prox;

        pAnterior->prox = pAtual;
        pAtual = pAnterior;
        
        // O ponteiro "pAnterior" foi atualizado para apontar para o próximo nó da lista
        pAnterior = proximo;
    }

    //No lugar de retornar true, atualizei a cabeça
    l->cabeca = pAtual;
}
